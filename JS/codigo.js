...
var pts = getParametroValor('puntos');
actualizarPuntos();
...
function drawRouletteWheel() {
  ...
  if (canvas.getContext) {
    ...
    ctx.font = "bold 14px 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif";

    for (var i = 0; i < options.length; i++) {
      var angle = startAngle + i * arc;

      if (i === 0) ctx.fillStyle = '#3BCD05';
      else if (i % 2 === 0) ctx.fillStyle = '#111111';
...
function spin() {
  ...
  spinTime = 0;
  spinTimeTotal = 6000; // tiempo girando
